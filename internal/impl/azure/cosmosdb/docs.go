...
type CRUDConfig struct {
	PartitionKeys   []*service.InterpolatedString
...
root."-" = if this.partition_keys.length() != 1 {
  ...
  "Only one partition key must be specified in the ` + "`partition_keys`" + ` field. Hierarchical partition keys are not currently supported."
}
`
...
func PartitionKeysField(isInterpolated bool) *service.ConfigField {
	desc := "Partition key values. Only string, integer and boolean values are supported."

	if isInterpolated {
		return service.NewInterpolatedStringListField(fieldPartitionKeys).Description(desc + " The partition key value type is deduced automatically when using interpolation.").Example("${! blobfish }")
	}

	return service.NewAnyListField(fieldPartitionKeys).Description(desc).Example("blobfish").Example("null")
}
...
func CRUDFields(hasReadOperation bool) []*service.ConfigField {
	...
	return []*service.ConfigField{
		...
		service.NewObjectListField(fieldPatchOperations, []*service.ConfigField{
			...
			service.NewInterpolatedStringField(fieldPatchValue).Description("Value.").Example(`${! json("foobar") }`).Example(`${! [1, 2, 3] }`).Optional(),
...
func CRUDConfigFromParsed(conf *service.ParsedConfig) (CRUDConfig, error) {
	...
	if c.PartitionKeys, err = conf.FieldInterpolatedStringList(fieldPartitionKeys); err != nil {
	...
	// - https://github.com/Azure/azure-sdk-for-go/issues/18578
	// - https://github.com/Azure/azure-sdk-for-go/issues/21063
	if len(c.PartitionKeys) != 1 {
		return CRUDConfig{}, errors.New("only one partition key is supported")
	}
	...
	if c.Operation == OperationPatch {
		...
		for _, poConf := range patchOperationsConfs {
			...
			if poConf.Contains(fieldPatchValue) {
				if po.Value, err = poConf.FieldInterpolatedString(fieldPatchValue); err != nil {
					return CRUDConfig{}, err
				}
			}
