...
func newCosmosDBReaderFromParsed(conf *service.ParsedConfig, mgr *service.Resources) (*cosmosDBReader, error) {
	...
	var partitionKeysConf []*service.ParsedConfig
	if partitionKeysConf, err = conf.FieldAnyList(cdbiFieldPartitionKeys); err != nil {
		return nil, err
	}
	...
	if len(partitionKeysConf) != 1 {
		return nil, errors.New("only one partition key is supported")
	}

	var partitionKeys []azcosmos.PartitionKey
	for _, pkConf := range partitionKeysConf {
		var pkValue any
		if pkValue, err = pkConf.FieldAny(); err != nil {
			return nil, err
		}

		typedPKValue, err := cosmosdb.GetTypedPartitionKeyValue(pkValue)
		if err != nil {
			return nil, err
		}

		partitionKeys = append(partitionKeys, typedPKValue)
	}
